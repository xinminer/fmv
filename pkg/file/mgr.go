...
func NewServer(conn server.NetConn, path string) Server {
	return &ConMgr{
		conn: conn,
		path: path,
...
func (c *ConMgr) handler() error {
	...
	for !c.conn.IsClose() {
		...
		case msg.MsgHead:
			...
			fs, err = os.OpenFile(filepath.Join(c.path, c.fileName), os.O_RDWR|os.O_CREATE|os.O_TRUNC, 0777)
			if err != nil {
...
func NewClient(conn server.NetConn, path string, files []string) Client {
	return &ConMgr{
		conn:       conn,
		path:       path,
		sendFiles:  files,
...
func (c *ConMgr) sendFile() error {
	...
	for _, file := range c.sendFiles {
		err = c.sendSingleFile(filepath.Join(c.path, file))
		if err != nil {
			return err
		}
	}
